...
class VisitSmallCavesOnce
  def can_visit?(path : Array(String), cave : CaveNode)
    cave.big? || !path.includes?(cave.name)
  end
end

class VisitSingleSmallCaveAtMostTwice
  def can_visit?(path : Array(String), cave : CaveNode)
    return true if cave.big?
    return false if cave.name == "start"
    return true if !path.includes?(cave.name)

    path
      .tally
      .select { |name, count| name[0, 1].downcase == name[0, 1] && count > 1 }
      .empty?
  end
end
...
class CaveMap
  ...
  def paths(source, destination, path_rules)
    ...
    while states.any?
      ...
      if name == destination
        ...
        options = node.connections.select { |cave| path_rules.can_visit?(path, cave) }

        states.concat(options.map { |cave| {cave.name, path.dup.push(cave.name)} })
      end
    end
